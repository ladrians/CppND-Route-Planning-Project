...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    int length = current_node->neighbors.size();
    for (int i = length - 1; i >= 0; i--)
    {
        RouteModel::Node *item = current_node->neighbors[i];
        item->parent = current_node;
        item->h_value = this->CalculateHValue(item);
        item->g_value =  current_node->distance(*item) + current_node->g_value;

        this->open_list.push_back(item);
        item->visited = true;
    }
}
...
/**
 * Reference from 03.17 - CODE: Expand the A* Search to Neighbors
 */
...
void RoutePlanner::AStarSearch() {
    ...
    while (RoutePlanner::end_node != current_node)
    {
